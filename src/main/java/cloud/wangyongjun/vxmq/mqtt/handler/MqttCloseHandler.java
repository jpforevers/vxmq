  ...
  public void run() {
    ...
    sessionService.getSession(mqttEndpoint.clientIdentifier())
      .onItem().transformToUni(session -> Uni.createFrom().voidItem()
        ...
        .onItem().call(v -> eventService.publishEvent(new MqttEndpointClosedEvent(Instant.now().toEpochMilli(), EventType.EVENT_MQTT_ENDPOINT_CLOSED_EVENT, VertxUtil.getNodeId(vertx), mqttEndpoint.clientIdentifier(), session.getSessionId()), false)))
